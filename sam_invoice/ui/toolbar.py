...
def create_toolbar(window: QMainWindow) -> QToolBar:
    ...
    window.act_articles = QAction(articles_icon, "Products", window)
    window.act_articles.setCheckable(True)
    ...
    window._icon_pairs = {
        ...
        window.act_articles: (articles_icon, articles_icon),
    ...
    toolbar.addAction(window.act_articles)
...
def set_active_toolbar_action(window: QMainWindow, action: QAction) -> None:
    ...
    """
    for act in (window.act_home, window.act_articles, window.act_invoices):
